...
crawl_date: "2025-11-16T08:00:22.465288Z"
selector: ".content"
---
...
| --- | --- | --- | --- |
| lock\_cmd | command to run when receiving a dbus lock event (e.g. `loginctl lock-session`) | string | empty |
| unlock\_cmd | command to run when receiving a dbus unlock event (e.g. `loginctl unlock-session`) | string | empty |
| on\_lock\_cmd | command to run when the session gets locked by a lock screen app | string | empty |
| on\_unlock\_cmd | command to run when the session gets unlocked by a lock screen app | string | empty |
| before\_sleep\_cmd | command to run when receiving a dbus prepare\_sleep event | string | empty |
| after\_sleep\_cmd | command to run when receiving a dbus post prepare\_sleep event | string | empty |
| ignore\_dbus\_inhibit | whether to ignore dbus-sent idle inhibit events (e.g. from firefox) | bool | false |
| ignore\_systemd\_inhibit | whether to ignore `systemd-inhibit --what=idle` inhibitors | bool | false |
| ignore\_wayland\_inhibit | whether to ignore Wayland protocol idle inhibitors | bool | false |
| inhibit\_sleep | sleep inhibition mode, 0 - disable, 1 - normal, 2 - auto, 3 - lock notify | int | 2 |
...
| ignore\_inhibit | ignore idle inhibitors (of all types) for this rule | bool | false |
